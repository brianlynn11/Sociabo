class DownloadsController < ApplicationController
  ...
  def new
    @download = Download.new(:contact => "US Phone or Email")
  ...
  def create
    ...
    # Tell the UserMailer to send Email after save
    if parseContact(@download[:contact]) == "email"
      UserMailer.welcome_email(@download[:contact]).deliver
    elsif parseContact(@download[:contact]) == "phone"
      UserMailer.sms_email("brian@sociabo.com", @download[:contact]).deliver
    end
